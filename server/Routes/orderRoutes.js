...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    if (!items || items.length === 0 || !totalAmount) {
      return res.status(400).json({ error: 'Order must contain items and a total amount.', code: 'INVALID_ORDER_DATA' });
    }
    const orderData = { orderNumber: uuidv4(), userId, items, subtotal, tax, shipping, totalAmount, currency, shippingAddress };
...
router.get('/my-orders', authenticateToken, async (req, res) => {
  try {
    const orders = await Order.find({ userId: req.user.id }).sort({ createdAt: -1 }).populate('paymentId');
...
router.patch('/:orderId/status', authenticateToken, async (req, res) => {
  try {
    ...
    const order = await Order.findOneAndUpdate({ _id: req.params.orderId, userId: req.user.id }, { status }, { new: true });
    if (!order) {
      return res.status(404).json({ error: 'Order not found', code: 'ORDER_NOT_FOUND' });
    }
