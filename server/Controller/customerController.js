...
export const createCustomer = async (req, res) => {
  try {
    ...
    // Checking if the users exists and has a Yuno customer ID
...
export const getCustomerPaymentMethods = async (req, res) => {
  try {
    ...
    const yunoResponse = await axios.get(
      `${YUNO_API_BASE}/customers/${customerId}/payment-methods`,
      {
        headers: {
          'accept': 'application/json',
          'public-api-key': process.env.PUBLIC_API_KEY,
    ...
    res.json({
      success: true,
      paymentMethods: yunoResponse.data
    });
    ...
    if (error.response) {
      const statusCode = error.response.status;
      const errorMessage = error.response.data?.message || 'Failed to get payment methods';
...
export const deleteCustomer = async (req, res) => {
  try {
    ...
    // Update local user record (remove Yuno customer ID)
    await User.findOneAndUpdate(
      { yunoCustomerId: customerId },
      { $unset: { yunoCustomerId: 1 } }
    );
